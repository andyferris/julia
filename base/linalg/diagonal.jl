...
@inline *(::Diagonal, ::RowVector) = error("Cannot right-multiply matrix by transposed vector")

@inline A_mul_Bt(D::Diagonal, rowvec::RowVector) = D*transpose(rowvec)

@inline At_mul_B(rowvec::RowVector, ::Diagonal) = error("Cannot left-multiply matrix by vector")

@inline A_mul_Bc(D::Diagonal, rowvec::RowVector) = D*ctranspose(rowvec)

@inline Ac_mul_B(rowvec::RowVector, ::Diagonal) = error("Cannot left-multiply matrix by vector")
