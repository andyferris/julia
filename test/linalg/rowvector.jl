...
@testset "Core" begin
    ...
    @test AbstractVector(rv)::Vector{Int} == [1, 2, 3]
    @test Vector{Float64}(rv)::Vector{Float64} == [1, 2, 3]
    @test RowVector{Float64,Vector{Float64}}(rv)::RowVector{Float64} == [1.0  2.0  3.0]
...
@testset "hcat" begin
    @test isa(hcat(), RowVector{Any})
    @test isa(Base.typed_hcat(Int), RowVector{Int})

    @test isa([1 2 3], RowVector{Int})
    @test isa(Float64[1 2 3], RowVector{Float64})

    @test isa([1 2 3.0], RowVector{Float64})
    @test isa(Float32[1 2 3.0], RowVector{Float32})
